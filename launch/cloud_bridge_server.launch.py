...
def generate_launch_description():
    ...
    config_dir, server_name, _, params_name = get_configs()

    # Get config filename
    config_filename = os.path.join(config_dir, server_name)
    param_filename = os.path.join(config_dir, params_name)
    param_list_params = get_param_list_params(param_filename)

    # Create our own temporary YAML files that include substitutions
    rewritten_list = ["manage_port", "sub_port", "pub_port", "req_port", "rep_port", "host_sub_port", "host_pub_port", "host_req_port", "host_rep_port"]
